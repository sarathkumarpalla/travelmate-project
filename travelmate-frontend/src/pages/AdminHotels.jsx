...
const EMPTY = { destinationId: '', name: '', location: '', rating: '', pricePerNight: '', imageUrl: '' };
...
const AdminHotels = () => {
    ...
    const fetchAll = async () => {
        try { setLoading(true); const data = await adminHotels.getAll(); setItems(data); }
    ...
    const save = async (e) => {
        ...
        try {
            if (editId) await adminHotels.update(editId, form);
            else await adminHotels.create(form);
            ...
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-4">
                        <Link to="/admin/dashboard" className="p-2 bg-white rounded-xl shadow-sm border border-slate-100 hover:bg-slate-50 transition-all"><ArrowLeft className="h-5 w-5 text-slate-500" /></Link>
                        <div><h1 className="text-3xl font-black text-slate-800">Hotels</h1><p className="text-slate-500 font-bold">{items.length} hotels in database</p></div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                        <table className="w-full">
                            <thead className="bg-slate-50">
                                <tr>
                                    {['IMAGE', 'NAME', 'LOCATION', 'RATING', 'PRICE/NIGHT', 'ACTIONS'].map(h => <th key={h} className="text-left p-4 text-sm font-black text-slate-500">{h}</th>)}
                                </tr>
                            </thead>
                            <tbody>
                                {items.map(item => (
                                    <tr key={item.id} className="border-t border-slate-50 hover:bg-slate-50/50">
                                        ...
                                        <td className="p-4 text-slate-500">₹{item.pricePerNight || '—'}</td>
                                ...
                                {items.length === 0 && <tr><td colSpan="6" className="text-center py-12 text-slate-400 font-bold">No hotels yet.</td></tr>}
                            </tbody>
                        </table>
                    </div>
                )}
                ...
                <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
                        <div className="flex justify-between items-center p-6 border-b border-slate-100">
                            <h2 className="text-xl font-black text-slate-800">{editId ? 'Edit' : 'Add'} Hotel</h2>
                        ...
                        <form onSubmit={save} className="p-6 space-y-4">
                            ...
                            <ImageUploadField label="Hotel Image" value={form.imageUrl} onChange={f('imageUrl')} />
                            <div>
                                <label className="block text-sm font-bold text-slate-600 mb-1">Hotel Name *</label>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="block text-sm font-bold text-slate-600 mb-1">Rating (0–5)</label><input type="number" step="0.1" min="0" max="5" value={form.rating} onChange={f('rating')} className="w-full p-3 border border-slate-100 rounded-xl bg-slate-50 outline-none focus:ring-2 focus:ring-emerald-300" /></div>
                                <div><label className="block text-sm font-bold text-slate-600 mb-1">Price Per Night (₹)</label><input type="number" value={form.pricePerNight} onChange={f('pricePerNight')} className="w-full p-3 border border-slate-100 rounded-xl bg-slate-50 outline-none focus:ring-2 focus:ring-emerald-300" /></div>
                            </div>
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="submit" disabled={saving} className="flex-1 py-3 bg-emerald-500 text-white rounded-xl font-bold hover:bg-emerald-600 transition-all flex items-center justify-center gap-2">
                                    {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : null}{saving ? 'Saving...' : 'Save Hotel'}
                                </button>
                            </div>
                        </form>
                    </div>
